...
module.exports.fetchAllOutcomes = ({ user_id }) => {
  ...
  const SQL_SELECT_OUTCOMES = `SELECT OUTCOME_ID, CATEGORY, DESCRIPTION, AMOUNT, TO_ACCOUNT, TO_BANK, BANK_ID, USER_ID
                                  FROM OUTCOME
                                  WHERE USER_ID = $1`;
...
module.exports.findOutcomeById = ({ user_id, outcome_id }) => {
  ...
  const SQL_SELECT_OUTCOME = `SELECT OUTCOME_ID, CATEGORY, DESCRIPTION, AMOUNT, TO_ACCOUNT, TO_BANK, BANK_ID, USER_ID
                                  FROM OUTCOME
                                  WHERE USER_ID = $1
                                  AND OUTCOME_ID = $2`;
